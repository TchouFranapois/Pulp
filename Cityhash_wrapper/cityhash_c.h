...
#ifdef CITYHASH_STATIC_DEFINE
#define CITYHASH_API  // Rien pour le mode statique
#else
#ifdef CITYHASH_WRAPPER_EXPORTS
...
extern "C" {
    ...
    CITYHASH_API uint64_t cityhash64(const char* buf, size_t len);
    CITYHASH_API uint64_t cityhash64_with_seed(const char* buf, size_t len, uint64_t seed);
